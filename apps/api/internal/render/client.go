...
// Render posts (design, data, theme) and returns the SVG. data is the merged,
// already-filtered snapshot map keyed by connector name. All failures are
// returned as *Error with a public-API-appropriate status:
// renderer 4xx (bad design) → 422, renderer 5xx / unreachable → 502.
func (c *Client) Render(ctx context.Context, design json.RawMessage, data map[string]any, theme string) (*Result, error) {
	if data == nil {
	...
	body, err := json.Marshal(renderRequest{Design: design, Data: data, Options: opts})
	if err != nil {
